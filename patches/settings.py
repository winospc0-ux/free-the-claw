...
class Settings(BaseSettings):
    ...
    @classmethod
    def populate_nim_settings(cls, data: Any) -> Any:
            ...
            for key, val in os.environ.items():
                if key.startswith(prefix):
                    ...
                    if field_name != "api_key":
                        nim_data[field_name] = val
            if nim_data:
                if "nim" not in data or data["nim"] is None:
                    data["nim"] = nim_data
                elif isinstance(data["nim"], dict):
                    data["nim"] = {**nim_data, **data["nim"]}
        return data
